# -*- coding: utf-8 -*-
"""
Created on Sun Apr 25 10:49:24 2021
...
def data_generator(filename_1, filename_2, folds_num,
    ...
    #Reshape from 784 vector  to 28x28
    for k in range (0,train_x.shape[0]):
        train_x[k] = np.reshape(train_x[k],(28,28))
    for k in range (0,test_x.shape[0]):
        test_x[k] = np.reshape(test_x[k],(28,28))
    print(train_x.shape)
    print(test_x.shape)
...
def main_function(filename_1, filename_2, folds_num, h, activation_hidden,
    ...
    for i in range(0, folds_num):
        ...
        # add 1st hidden layer
        model.add(layers.Dense(h[0], activation=activation_hidden))
        # add the rest hidden layers
        for n in range(1, len(h)):
            model.add(layers.Dense(h[n], activation=activation_hidden))
        # add final layer
...
if __name__ == "__main__":
    ...
    main_function(train_filename, test_filename, folds_num, h, activation_hidden,
                  activation_output, optimizer, loss, epochs, batch_size, lr, momentum)
